<template>
  <div class="page-container">
    <h1 class="page-title">
      <el-icon :size="22"><DataAnalysis /></el-icon> 销售统计
    </h1>
    ...
    <el-row :gutter="20">
      <el-col :span="12">
        <el-card><template #header><h3><el-icon :size="16"><TrendCharts /></el-icon> 7日收入趋势</h3></template>
          <Line v-if="trendReady" :data="trendData" :options="chartOpts" />
          <el-empty v-else description="暂无数据" :image-size="80" />
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card><template #header><h3><el-icon :size="16"><Odometer /></el-icon> 菜品销量 Top 5</h3></template>
    ...
    <el-row :gutter="20" style="margin-top:20px">
      <el-col :span="12">
        <el-card><template #header><h3><el-icon :size="16"><DataAnalysis /></el-icon> 订单状态分布</h3></template>
          <Pie v-if="pieReady" :data="pieData" :options="chartOpts" />
          <el-empty v-else description="暂无数据" :image-size="80" />
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card><template #header><h3><el-icon :size="16"><Document /></el-icon> 状态汇总</h3></template>
...
const trendData = ref({ labels: [], datasets: [{ label: '收入(元)', data: [], borderColor: '#FF8C00', backgroundColor: 'rgba(255,140,0,0.1)', fill: true }] })
const topData = ref({ labels: [], datasets: [{ label: '销量', data: [], backgroundColor: ['#f56c6c','#e6a23c','#FF8C00','#67c23a','#909399'] }] })
const pieData = ref({ labels: [], datasets: [{ data: [], backgroundColor: ['#f56c6c','#e6a23c','#FF8C00','#67c23a','#909399','#ff9800','#9c27b0'] }] })
...
async function fetchData() {
  try {
    ...
    trendData.value = { labels: days.map(d => d.substring(5)), datasets: [{ label: '收入(元)', data: dayRevenue, borderColor: '#FF8C00', backgroundColor: 'rgba(255,140,0,0.1)', fill: true }] }
    ...
    topData.value = { labels: top.map(t => t[0]), datasets: [{ label: '销量', data: top.map(t => t[1]), backgroundColor: ['#f56c6c','#e6a23c','#FF8C00','#67c23a','#909399'] }] }
    topReady.value = true
    ...
    pieData.value = { labels: sl, datasets: [{ data: sc, backgroundColor: ['#f56c6c','#e6a23c','#FF8C00','#67c23a','#909399','#ff9800'] }] }
    pieReady.value = true
